// specter_020: Direct best-move tracking + king-push-awareness eval
//
// Key insight: avoid TT for root best move — track best_move DIRECTLY during
// iterative deepening. The root search loop explicitly tracks which move scored
// best. The `best` variable is updated ONLY from the root search loop, never
// from TT.
//
// Eval additions beyond chimera:
//   - King push shield: +10 per friendly piece adjacent to own king
//   - King push escape: +15 when king has push escape routes
//
// Search params (conservative, chimera-like):
//   - LMR divisor 1.8
...
impl Specter {
    ...
    // Chimera eval + king-push-awareness
    ...
    // search() does NOT have is_pv (like chimera)
    fn search(
        &mut self,
        pos: &mut Position,
        depth: i32,
        ...
        for i in 0..ml.moves.len() {
            ...
            if i >= 3 && depth >= 2 && !is_tactical && !gives_check && !in_check {
                ...
                let s0 = -self.search(pos, depth - 1 - r, -(alpha + 1), -alpha, ply + 1, gives_check);
                if s0 > alpha && !self.stopped {
                    self.lmr_re += 1;
                    score = -self.search(pos, depth - 1, -beta, -alpha, ply + 1, gives_check);
                } else {
                    score = s0;
                }
            } else {
                ...
                if i > 0 && !self.stopped {
                    let s1 = -self.search(pos, depth - 1 + ext, -(alpha + 1), -alpha, ply + 1, gives_check);
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList, stats: &mut SearchStats) {
        ...
        let mut diag = format!(
            r#"{{"engine":"specter_020","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
        ...
        for depth in 1..=30 {
            ...
            loop {
                ...
                for i in 0..root.moves.len() {
                    ...
                    let score;
                    if i > 0 && !self.stopped {
                        let s1 = -self.search(pos, depth - 1, -(alpha + 1), -alpha, 1, gives_check);
                        if s1 > alpha && s1 < beta && !self.stopped {
                            score = -self.search(pos, depth - 1, -beta, -alpha, 1, gives_check);
                        } else {
                            score = s1;
                        }
                    } else {

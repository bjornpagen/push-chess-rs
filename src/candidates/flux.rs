// flux_022: Maximum pruning for maximum depth
//
// Hypothesis: Push more aggressively on every pruning parameter to get
// the deepest search possible. Chimera eval is proven -- don't touch it.
// Instead, push LMR divisor to 1.3 (most aggressive ever), LMP very tight,
// futility to depth 6, razoring at depth 3. The engine that sees deepest wins.
//
// Key params:
//   - LMP: depth <= 4, prune at 4 + depth*2 (very aggressive)
//   - Futility: depth <= 6, margin = depth * 150
//   - Razoring: depth <= 3, margin 400
//   - Aspiration: 30cp window
//   - Check extension up to depth 8
//   - TT: 512K
//   - Chimera eval (proven, untouched)
...
// -----------------------------------------------------------------------
// LMR table -- divisor 1.3 (most aggressive)
// -----------------------------------------------------------------------
    ...
    for d in 0..32 {
        for m in 0..256 {
            t[d][m] = if d < 2 || m < 3 {
                ...
                (0.5 + (d as f64).ln() * (m as f64).ln() / 1.3) as i32
            };
        }
    }
    t
...
impl Flux {
    ...
    fn evaluate(&self, pos: &Position) -> i32 {
        ...
        // King safety (chimera values, untouched)
        for ci in 0..2usize {
            let c: Color = if ci == 0 { Color::White } else { Color::Black };
            let sign: i32 = if c == stm { 1 } else { -1 };
            let kr = (king[ci] >> 3) as i32;
            ...
            // Exposed king: -40 if <2 pawns near king
    ...
    fn search(
        ...
        while i < ml.moves.len() && !self.stopped {
            ...
            // Very aggressive LMP: depth <= 4, prune at 4 + depth*2
            ...
            if i >= 3 && depth >= 2 && !is_tactical && !gives_check && !in_check {
                ...
                // Check extension up to depth 8
                let ext = if gives_check && depth <= 8 { 1 } else { 0 };
                if i > 0 && !self.stopped {
                    let s = -self.search(
                        pos,
                        depth - 1 + ext,

// pulse_001: Cheap mobility proxy + lazy eval + optimized wraith
//
// Based on wraith_001 (specter+tempest hybrid) with:
//   1. Cheap mobility proxy: centrality bonus for non-pawn pieces instead of
//      counting actual moves. Open file bonus for rooks. Knight outpost bonus.
//   2. Lazy eval cutoff: material-only eval for reverse futility at shallow
//      depths, skipping expensive positional eval when margin is large.
//   3. History table as flat Vec<i16> with manual indexing for cache locality.
//   4. Fresh move validation at end of choose_move.
//
// Keeps from wraith:
...
//   - NEON find_max_index
//   - All search features (LMR, LMP, IID, aspiration, killer/history/countermove)
//   - Ply >= 128 guard
//   - Vec<Move> MoveList
...
// ---------------------------------------------------------------------------
// History table: flat Vec<i16> with manual indexing for cache locality
// Index: color * 64 * 64 + from * 64 + to
// ---------------------------------------------------------------------------
...
impl PulseEngine {
    ...
    // Lazy evaluate: material only (cheap, for reverse futility pre-check)
    fn lazy_evaluate(&self, pos: &Position) -> i32 {
        ...
        for sq in 0u8..64 {
            ...
            let sign: i32 = if piece.color == stm { 1 } else { -1 };
            sc += sign * pval(piece.piece_type);
        }
        sc
    }

    // Full evaluate: material + positional + cheap mobility + push awareness
    fn evaluate(&self, pos: &Position) -> i32 {
        ...
        let mut pawn_files = [[false; 8]; 2]; // track which files have pawns per side
        ...
        // Track knight squares for outpost detection
        let mut knight_sq = [[0u8; 4]; 2];
        let mut knight_n = [0usize; 2];

        // Track rook squares for open file detection
        let mut rook_sq = [[0u8; 4]; 2];
        ...
        for sq in 0u8..64 {
            ...
            match pt {
                PieceType::Knight => {
                    sc_pos += sign * (3 - cd) * 5; // original centrality
                    sc_pos += sign * (4 - cd) * 4; // mobility proxy: weight=4 for knights
                    if knight_n[ci] < 4 {
                        knight_sq[ci][knight_n[ci]] = sq;
                        knight_n[ci] += 1;
                    }
                }
                PieceType::Bishop => {
                    sc_pos += sign * (3 - cd) * 2; // original centrality
                    sc_pos += sign * (4 - cd) * 3; // mobility proxy: weight=3 for bishops
                }
                PieceType::Rook => {
                    sc_pos += sign * (3 - cd) * 2;
                    sc_pos += sign * (4 - cd) * 2; // mobility proxy: weight=2 for rooks
                    if rook_n[ci] < 4 {
                ...
                PieceType::Queen => {
                    sc_pos += sign * (3 - cd) * 2;
                    sc_pos += sign * (4 - cd) * 2; // mobility proxy: weight=2 for queens
                }
                _ => {}
            }
        ...
        for ci in 0..2 {
            ...
            for ri in 0..rook_n[ci] {
                ...
                if !pawn_files[ci][rf] {
                    if !pawn_files[oci][rf] {
                        sc_pos += sign * 25; // fully open file
                    } else {
                        sc_pos += sign * 15; // semi-open file
                    }
                }
            }
        }
        ...
        // === KNIGHT OUTPOST ===
        // +20 for knight on rank 4-5 defended by friendly pawn, can't be attacked by enemy pawns
        for ci in 0..2 {
            let c = if ci == 0 { Color::White } else { Color::Black };
            let sign: i32 = if c == stm { 1 } else { -1 };
            let _oci = 1 - ci;
            for ni in 0..knight_n[ci] {
                let nsq = knight_sq[ci][ni];
                let nr = (nsq >> 3) as i32;
                let nf = (nsq & 7) as i32;
                ...
                while cr >= 0 && cr <= 7 {
                    for df in [-1i32, 1] {
                        ...
                        let cp = pos.board[cs];
                        if cp.piece_type == PieceType::Pawn && cp.color != c {
                            // This enemy pawn could advance to attack the knight
                            // Check if it's on an adjacent file and could reach a square
                            // that attacks the knight
    ...
    fn search(
        ...
        // === LAZY EVAL CUTOFF for reverse futility ===
        // At shallow depths, compute material-only eval first.
        // If material alone is far above beta, cut immediately without full eval.
        // If material is close, compute full eval for the standard reverse futility check.
        if !in_check && !is_pv && depth <= 4 && ply > 0 {
            let lazy_eval = self.lazy_evaluate(pos);

            // Large margin: if material alone exceeds beta by a lot, skip full eval
            ...
            // Standard reverse futility: only compute full eval if material is in range
            if lazy_eval - depth * 100 >= beta {
                // Material is above threshold, but let's verify with full eval
                let eval = self.evaluate(pos);
                if eval - depth * 100 >= beta {
                    self.null_cuts += 1;
                    return beta;
                }
            } else if lazy_eval + depth * 100 < beta {
                // Material is way below beta, do normal futility with full eval
                let eval = self.evaluate(pos);
                if eval - depth * 100 >= beta {
                    self.null_cuts += 1;
                    return beta;
                ...
                // Material is close to the threshold - need full eval
                let eval = self.evaluate(pos);
                if eval - depth * 100 >= beta {
                    self.null_cuts += 1;
                    return beta;

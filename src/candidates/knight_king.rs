// knight_king_001: Push-cascade king safety specialist
//
// Based on phantom_018 (megabrawl champion, 78.9%).
// Adds push-cascade king safety evaluation: penalizes positions where
// enemy rooks/queens are aligned with the friendly king on a file or rank
// with an unbroken chain of friendly pieces between them (push threat).
// All phantom features preserved: king zone attacks, rook bonuses, pawn structure,
// piston bonus, LMR divisor 1.8, aspiration windows.
...
impl KnightKing {
    ...
    fn evaluate(&self, pos: &Position) -> i32 {
        ...
        // === PUSH-CASCADE KING SAFETY (knight_king innovation) ===
        // For each side, scan outward from the king along rook directions (files/ranks).
        // If there is an unbroken chain of friendly pieces leading to an enemy rook/queen,
        // that slider could push the chain into/through the king. Penalize accordingly.
        for ci in 0..2usize {
            let c = if ci == 0 { Color::White } else { Color::Black };
            ...
            // Check all 4 rook directions outward from the king
            for &(dr, df) in &[(0i32, 1i32), (0, -1), (1, 0), (-1, 0)] {
                let mut friendly_between = 0i32;
                let mut r = kr + dr;
                let mut f = kf + df;
                ...
                while r >= 0 && r < 8 && f >= 0 && f < 8 {
                    ...
                    let p = pos.board[sq];
                    if p.is_empty() {
                        break; // chain broken by empty square
                    } else if p.color == c {
                        friendly_between += 1;
                    } else {
                        // Enemy piece found
                        if (p.piece_type == PieceType::Rook || p.piece_type == PieceType::Queen)
                            && friendly_between > 0
                        {
                            // Enemy slider with unbroken pushable chain toward king
                            let penalty = match friendly_between {
                                1 => 15,
                                2 => 35,
                                _ => 50,
                            };
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList) -> String {
        ...
        let mut s = format!(
            r#"{{"engine":"knight_king_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
...
impl Engine for KnightKing {
    fn name(&self) -> &str { "knight_king" }

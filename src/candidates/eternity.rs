...
impl EternityEngine {
    ...
    fn tt_store(&mut self, key: u64, depth: i32, score: i32, flag: u8, m: &Move) {
        ...
        if e.key32 == 0 || e.key32 == k32 || depth >= e.depth as i32 || e.age != self.tt_age {
            e.key32 = k32;
    ...
    fn order_moves(&self, pos: &Position, ml: &mut MoveList, ply: usize, ttm: &Move) {
        ...
        for i in 0..ml.len() {
            ...
            if !tp.is_empty() {
                if tp.color == stm {
                    let ph = self.push_history
                        [history_idx(stm as usize, m.from as usize, m.to as usize)]
                        as i32;
                    if ph >= 0 {
                        sv = 1_000_000 + pval(tp.piece_type) * 10 + ph;
                    } else {
                        sv = 300_000 + ph;
                    } // Demoted below killers
                } else {
                ...
                if ply < 64 && *m == self.killers[ply][0] {
                    ...
                    sv = self.history[history_idx(stm as usize, m.from as usize, m.to as usize)]
                        as i32
                        + pst_value(mpt, stm, m.to)
                        - pst_value(mpt, stm, m.from);
                }
            }
    ...
    fn search(
        ...
        for i in 0..ml.len() {
            ...
            let push_hist = if is_push {
                self.push_history[history_idx(stm as usize, m.from as usize, m.to as usize)]
            } else {
                0
            };
            ...
            if i >= 3 && depth >= 2 && !is_tactical && !gives_check && !in_check {
                ...
                let hs = if is_push {
                    self.push_history[history_idx(ci, m.from as usize, m.to as usize)] as i32
                } else {
                    self.history[history_idx(ci, m.from as usize, m.to as usize)] as i32
                };
                if hs < -500 {
                    r += 2;
                } else if hs < -100 {
            ...
            if alpha >= beta {
                ...
                if is_push {
                    let h = &mut self.push_history[history_idx(ci, m.from as usize, m.to as usize)];
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList, stats: &mut SearchStats) {
        ...
        let mut diag = format!(
            r#"{{"engine":"eternity_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"max_qdep":{},"hashfull":{},"cut_hist":[{},{},{},{},{},{}],"cut_types":{{"cap":{},"push":{},"quiet":{}}},"top_moves":["#,
            self.qnodes,
            self.tt_hits,
            self.beta_cuts,
            self.first_cuts,

// inferno_001: Phantom's eval + Ember's pawn structure
//
// Combines phantom_018's king zone attack scoring, rook bonuses, and full eval
// with ember's pawn chain defense, connected pawns, pawn storm, and rook-pawn synergy.
// Search unchanged from phantom: conservative LMR 1.8, TT-based root, NEON move ordering.
...
// LMR table: divisor 1.8 (same as chimera/phantom)
...
impl Inferno {
    ...
    fn evaluate(&self, pos: &Position) -> i32 {
        ...
        for ci in 0..2usize {
            ...
            for si in 0..slider_n[ci] {
                ...
                else if !friendly_pawn { sc_pos += sign * 25; } // semi-open: increased from 15 to 25 (ember emphasis)
        ...
        // === PAWN STRUCTURE (phantom original) ===
        for ci in 0..2usize {
            let c = if ci == 0 { Color::White } else { Color::Black };
            let sign = if c == stm { 1 } else { -1 };
            for pi in 0..pawn_n[ci] {
        ...
        // === EMBER PAWN STRUCTURE ADDITIONS ===
        for ci in 0..2usize {
            let c = if ci == 0 { Color::White } else { Color::Black };
            let sign = if c == stm { 1 } else { -1 };
            for pi in 0..pawn_n[ci] {
                ...
                // Pawn chain bonus: friendly pawn diagonally behind (defending this pawn)
                let behind_rank = if c == Color::White { pr - 1 } else { pr + 1 };
                if behind_rank >= 0 && behind_rank < 8 {
                    for df in [-1i32, 1] {
                        let bf = pf + df;
                        if bf < 0 || bf > 7 { continue; }
                        let bsq = (behind_rank * 8 + bf) as usize;
                        let bp = pos.board[bsq];
                        if !bp.is_empty() && bp.piece_type == PieceType::Pawn && bp.color == c {
                            sc_pos += sign * 15;
                            break; // count at most once per pawn
                        }
                    }
                }

                // Connected pawns: friendly pawn on adjacent file AND adjacent rank (diagonal neighbor)
                for df in [-1i32, 1] {
                    let nf = pf + df;
                    ...
                    for dr in [-1i32, 1] {
                        ...
                        let np = pos.board[nsq];
                        if !np.is_empty() && np.piece_type == PieceType::Pawn && np.color == c {
                            sc_pos += sign * 12;
                            // Only count one connection per direction pair; break inner
                            break;
                        }
                    }
                }
            ...
            // Pawn storm: bonus for advanced pawns near enemy king wing
            let oci = 1 - ci;
            let ekf = (king[oci] & 7) as i32;
            let on_wing = ekf <= 2 || ekf >= 5; // files a-c or f-h
            if on_wing {
                for pi in 0..pawn_n[ci] {
                    let psq = pawn_sq[ci][pi];
                    let pr = (psq >> 3) as i32;
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList) -> String {
        ...
        let mut s = format!(
            r#"{{"engine":"inferno_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
...
impl Engine for Inferno {
    fn name(&self) -> &str { "inferno" }
    ...
    fn choose_move(&mut self, pos: &mut Position, budget: &SearchBudget) -> (Move, SearchStats) {
        ...
        for depth in 1..=30 {
            ...
            if depth <= 3 || best_score.abs() > 5000 {
                ...
                alpha = best_score - 50;  // aspiration window 50cp
                beta = best_score + 50;
            }

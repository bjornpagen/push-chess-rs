// apex_001: Phantom + singular extension
//
// Based on phantom_018 (megabrawl champion, 78.9%, 12W-6D-1L).
// Adds singular extension: uniquely good TT moves get extended by 1 ply,
// allowing deeper search on critical lines (technique from Stockfish).
...
impl Apex {
    ...
    fn evaluate(&self, pos: &Position) -> i32 {
        ...
        for ci in 0..2usize {
            ...
            if pawns_near_king < 2 { sc_pos += sign * (-50); } // stronger penalty
            ...
            if !has_pawn_on_file { sc_pos += sign * (-30); } // stronger penalty

            if bishop_count[ci] >= 2 { sc_pos += sign * 40; } // stronger bishop pair
        }
        ...
        for ci in 0..2usize {
            ...
            for si in 0..slider_n[ci] {
                ...
                if !friendly_pawn && !enemy_pawn { sc_pos += sign * 25; } // fully open
                else if !friendly_pawn { sc_pos += sign * 15; } // semi-open

                // Connected rooks (same rank)
                if prev_rook_rank == sr { sc_pos += sign * 15; }
                prev_rook_rank = sr;
            }
        }
    ...
    fn search(&mut self, pos: &mut Position, depth: i32, mut alpha: i32, mut beta: i32, ply: i32, in_check: bool) -> i32 {
        ...
        // Singular extension: if TT move exists with sufficient depth and
        // is much better than alternatives, extend it by 1 ply
        let mut singular_extend = 0i32;
        if !move_is_null(&ttm) && tt_depth as i32 >= depth - 3 && tt_flag != 1 && ply > 0
            && move_is_null(&self.excluded_move)
        {
            // Search all moves EXCEPT ttm at reduced depth with a narrow window
            // to see if any alternative is close to the TT score
            let se_beta = tt_score - 2 * depth; // singular margin
            let se_depth = (depth - 1) / 2;     // half depth

            // Do a reduced search excluding the TT move
            // Set a flag so the move loop skips ttm
            self.excluded_move = ttm;
            let se_score = self.search(pos, se_depth, se_beta - 1, se_beta, ply, in_check);
            self.excluded_move = Move::default();

            if se_score < se_beta {
                singular_extend = 1; // TT move is singular -- extend it
            }
        }
        ...
        if ml.moves.len() == 0 { return if in_check { -99000 + ply } else { 0 }; }
        ...
        let mut i = 0;
        let mut move_index = 0; // actual move index (excluding skipped excluded_move)
        while i < ml.moves.len() && !self.stopped {
            let m = ml.moves[i];

            // Skip excluded move during singular extension verification search
            if !move_is_null(&self.excluded_move) && self.excluded_move == m {
                i += 1;
                continue;
            }
            ...
            if !is_tactical && !in_check && depth <= 2 && move_index as i32 >= 8 + depth * 4 {
                i += 1; move_index += 1; continue;
            }
            ...
            let mi = move_index.min(255);

            if move_index >= 3 && depth >= 2 && !is_tactical && !gives_check && !in_check {
                self.lmr_tries += 1;
                ...
                if score > alpha && !self.stopped {
                    ...
                    if alpha >= beta {
                        ...
                        if move_index == 0 { self.first_cuts += 1; }
                ...
                // Check extension (chimera: depth <= 4) + singular extension for TT move
                let check_ext = if gives_check && depth <= 4 { 1 } else { 0 };
                let ext = check_ext + if move_index == 0 { singular_extend } else { 0 };
                if move_index > 0 && !self.stopped {
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList) -> String {
        ...
        let mut s = format!(
            r#"{{"engine":"apex_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
...
impl Engine for Apex {
    fn name(&self) -> &str { "apex" }
    ...
    fn choose_move(&mut self, pos: &mut Position, budget: &SearchBudget) -> (Move, SearchStats) {
        ...
        if root.moves.len() == 0 { return (Move::default(), stats); }
        if root.moves.len() == 1 {
        ...
        // Move validation with fresh legal moves

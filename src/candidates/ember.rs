// ember_001: Pawn structure specialist with chain defense + storm attack
//
// Base: specter_020 (tournament champion) with direct best-move tracking
// + king-push-awareness eval.
//
// New eval terms:
//   - Doubled pawns:  -15 per extra pawn on same file
//   - Isolated pawns: -20 for pawn with no friendly pawns on adjacent files
//   - Connected pawns: +12 for pawn with friendly pawn on adjacent file+rank (diagonal)
//   - Pawn chain:     +15 for each pawn defended by another pawn diagonally behind
//   - Pawn storm:     +10 per advanced (past rank 4) pawn near enemy king side
//   - Rook-pawn synergy: +30 semi-open file (enemy pawns only), +20 fully open file
//
// Search additions:
//   - PV-node TT restriction (is_pv parameter, like tempest)
//
// Kept from specter: king-push-awareness, direct best-move tracking, NEON,
//   conservative LMR (1.8), LMP, futility, check ext, aspiration 50cp, TT 512K
...
impl EmberEngine {
    ...
    // Evaluate: chimera base + king-push-awareness (specter) + pawn structure (ember)
    fn evaluate(&self, pos: &Position) -> i32 {
        ...
        let mut pawn_file_count = [[0i32; 8]; 2]; // [color][file] -> count of pawns

        // Track rook squares for rook-pawn synergy
        let mut rook_sq = [[0u8; 8]; 2];
        ...
        for sq in 0u8..64 {
            ...
            if pt == PieceType::Pawn {
                ...
                pawn_file_count[ci][f as usize] += 1;
                let mut passed = true;
            ...
            if pt == PieceType::Rook && rook_n[ci] < 8 {
                rook_sq[ci][rook_n[ci]] = sq;
                rook_n[ci] += 1;
            }
        ...
        // === EMBER: PAWN STRUCTURE EVALUATION ===
        for ci in 0..2 {
            ...
            // Doubled pawns: -15 per extra pawn on same file
            for file in 0..8 {
                let cnt = pawn_file_count[ci][file];
                if cnt > 1 {
                    sc_pos += sign * (-15 * (cnt - 1));
                }
            }

            // Per-pawn structure bonuses
            for pi in 0..pawn_n[ci] {
                let psq = pawn_sq[ci][pi];
                let pf = (psq & 7) as i32;
                let pr = (psq >> 3) as i32;

                // Isolated pawns: no friendly pawns on adjacent files -> -20
                let has_adj_file_pawn = (pf > 0 && pawn_file_count[ci][(pf - 1) as usize] > 0)
                    || (pf < 7 && pawn_file_count[ci][(pf + 1) as usize] > 0);
                if !has_adj_file_pawn {
                    sc_pos += sign * (-20);
                }

                // Connected pawns: friendly pawn on adjacent file AND adjacent rank -> +12
                // Check all 4 diagonal neighbors for a friendly pawn
                ...
                for df in [-1i32, 1] {
                    ...
                    for dr in [-1i32, 1] {
                        ...
                        let np = pos.board[ns];
                        if np.piece_type == PieceType::Pawn && np.color == c {
                            connected = true;
                        }
                    }
                }
                ...
                // Pawn chain bonus: pawn defended by another pawn diagonally behind -> +15
                // "Behind" means toward own back rank
                let back_dir: i32 = if c == Color::White { -1 } else { 1 };
                let br = pr + back_dir;
                if (0..=7).contains(&br) {
                    for df in [-1i32, 1] {
                        let bf = pf + df;
                        ...
                        let bp = pos.board[bs];
                        if bp.piece_type == PieceType::Pawn && bp.color == c {
                            sc_pos += sign * 15;
                            break; // count once per defended pawn
                        }
                    }
                }
            }
        ...
        // === EMBER: PAWN STORM BONUS ===
        // When enemy king is on one side (files a-c or f-h), bonus for own pawns
        // advanced past rank 4 on adjacent files to enemy king
        for ci in 0..2 {
            let c = if ci == 0 { Color::White } else { Color::Black };
            let oci = 1 - ci;
            let sign: i32 = if c == stm { 1 } else { -1 };
            let enemy_kf = (king[oci] & 7) as i32;

            // Only apply storm if enemy king is on a side (files 0-2 or 5-7)
            if enemy_kf <= 2 || enemy_kf >= 5 {
                for pi in 0..pawn_n[ci] {
                    let psq = pawn_sq[ci][pi];
                    let pf = (psq & 7) as i32;
                    let pr = (psq >> 3) as i32;
                    ...
                    // Pawn must have advanced past rank 4
                    if adv <= 4 {
                        continue;
                    }

                    // Pawn must be on an adjacent file to enemy king
                    let file_dist = (pf - enemy_kf).abs();
                    if file_dist <= 1 {
                        sc_pos += sign * 10;
                    }
                }
        ...
        // === EMBER: ROOK-PAWN SYNERGY ===
        // Semi-open file (no friendly pawns, has enemy pawns): +30
        // Fully open file (no pawns at all): +20
        for ci in 0..2 {
            let c = if ci == 0 { Color::White } else { Color::Black };
            let oci = 1 - ci;
            let sign: i32 = if c == stm { 1 } else { -1 };

            for ri in 0..rook_n[ci] {
                let rsq = rook_sq[ci][ri];
                let rf = (rsq & 7) as usize;

                let friendly_pawns_on_file = pawn_file_count[ci][rf];
                let enemy_pawns_on_file = pawn_file_count[oci][rf];

                if friendly_pawns_on_file == 0 {
                    if enemy_pawns_on_file > 0 {
                        // Semi-open: rook can push enemy pawns
                        sc_pos += sign * 30;
                    } else {
                        // Fully open file
                        sc_pos += sign * 20;
        ...
        // Piston bonus (from specter/chimera)
        for ci in 0..2 {
            let c = if ci == 0 { Color::White } else { Color::Black };
            let sign: i32 = if c == stm { 1 } else { -1 };
            for pi in 0..pawn_n[ci] {
        ...
        // King tropism (from specter/chimera)
        for ci in 0..2 {
            let oci = 1 - ci;
        ...
        // King safety (chimera base + specter king-push-awareness)
        for ci in 0..2 {
            let c = if ci == 0 { Color::White } else { Color::Black };
            let sign: i32 = if c == stm { 1 } else { -1 };
            let kr = (king[ci] >> 3) as i32;
            ...
            // === KING PUSH AWARENESS (from specter) ===
    ...
    fn search(
        ...
        for i in 0..ml.moves.len() {
            ...
            if i >= 3 && depth >= 2 && !is_tactical && !gives_check && !in_check {
                ...
                // LMR reduced search: null-window, not PV
                let s0 = -self.search(
                    pos,
                    depth - 1 - r,
                    -(alpha + 1),
                ...
                if i > 0 && !self.stopped {
                    ...
                    // First move or full-window: preserve is_pv
                    score = -self.search(
                        pos,
                        depth - 1 + ext,
                        -beta,
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList, stats: &mut SearchStats) {
        ...
        let mut diag = format!(
            r#"{{"engine":"ember_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
            self.qnodes,
            self.tt_hits,
            self.beta_cuts,
            self.first_cuts,

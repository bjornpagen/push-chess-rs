...
//   1. Stack-allocated MoveList with embedded i32 scores — no heap allocs
//      for move ordering. Simple scalar selection sort (no NEON).
//   2. Integer scoring throughout order_moves (eliminates f32 entirely).
//   3. Stack-allocated tactical array in qsearch.
//   4. Post-search move validation: re-generates legal moves and verifies
//      the chosen best move is among them, falling back to first legal move.
//
// Preserved from wraith:

// dynamo_001: Countermove history table
//
// Based on phantom_018. Replaces simple countermove table with a countermove
// history scoring table indexed by [prev_to][cur_from][cur_to], providing
// richer move ordering signal for quiet moves that respond to opponent moves.
...
struct Dynamo {
    ...
    cmh: Vec<i16>,  // countermove history: size 64 * 64 * 64 = 262144, indexed by prev_to * 4096 + cur_from * 64 + cur_to
    prev_move: Move,
    move_buf: Vec<Move>,
    nodes: u64,
    qnodes: u64,
...
impl Dynamo {
    ...
    fn order_moves(&self, pos: &Position, ml: &mut MoveList, ply: usize, ttm: &Move) {
        ...
        let has_prev = self.prev_move.from != 0 || self.prev_move.to != 0;
        ...
        for i in 0..ml.moves.len() {
            ...
            if *m == *ttm { s = 1e7; }
                ...
                // Countermove history score
                if has_prev {
                    let cmh_idx = self.prev_move.to as usize * 4096 + m.from as usize * 64 + m.to as usize;
                    v += self.cmh[cmh_idx] as f32;
                }
                s = v as f64;
            }
            scores[i] = s as f32;
    ...
    fn update_cmh(&mut self, saved_prev: &Move, m: &Move, depth: i32, ml: &MoveList, i: usize, pos: &Position) {
        if saved_prev.from == 0 && saved_prev.to == 0 { return; }
        let cmh_idx = saved_prev.to as usize * 4096 + m.from as usize * 64 + m.to as usize;
        self.cmh[cmh_idx] = (self.cmh[cmh_idx] as i32 + depth * depth).min(16000) as i16;
        // Penalize other quiet moves in CMH
        for j in 0..i {
            if pos.board[ml.moves[j].to as usize].is_empty() {
                let ci2 = saved_prev.to as usize * 4096 + ml.moves[j].from as usize * 64 + ml.moves[j].to as usize;
                self.cmh[ci2] = (self.cmh[ci2] as i32 - depth).max(-16000) as i16;
            }
        }
    }
                    ...
                    if alpha >= beta {
                        ...
                        if !is_tactical && (ply as usize) < 64 {
                            ...
                            *h = (*h + (depth * depth) as i16).min(16000);
                            ...
                            self.update_cmh(&saved_prev, &m, depth, &ml, i, pos);
                        }
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList) -> String {
        ...
        let mut s = format!(
            r#"{{"engine":"dynamo_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
...
impl Engine for Dynamo {
    fn name(&self) -> &str { "dynamo" }

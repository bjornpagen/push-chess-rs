// wraith_001: Specter+Tempest hybrid with push vulnerability defense
//
// Based on specter_020 (tournament champion) with two key additions:
//   1. PV-node awareness from tempest: is_pv parameter in search().
//      - No TT cutoffs in PV nodes
//      - No reverse futility pruning in PV nodes
//      - First child of PV node searched with full window (is_pv=true),
//        subsequent children use zero-window (is_pv=false)
//   2. Push vulnerability penalty: -20 per piece that sits on a file/rank
//      where an enemy slider (rook/queen) is aimed at it with friendly
//      pieces behind it (forming a push chain the enemy can exploit).
//   3. TT enlarged to 1M entries (1 << 20)
//
...
//   - Aspiration 50cp
//   - Killer/history/countermove infrastructure
...
impl WraithEngine {
    ...
    // Chimera eval + king-push-awareness + push vulnerability penalty
    fn evaluate(&self, pos: &Position) -> i32 {
        ...
        // For each side, scan for pieces that sit in "push-vulnerable" positions:
        // a piece on a file/rank where an enemy slider (rook/queen) is aimed at it
        // with friendly pieces behind it (forming a push chain the enemy can exploit).
        // Penalty: -20 per vulnerable piece.
        for ci in 0..2 {
            let c = if ci == 0 { Color::White } else { Color::Black };
            let sign: i32 = if c == stm { 1 } else { -1 };
            let oci = 1 - ci;

            // For each piece of color c, check if an enemy slider aims at it
            // along a file or rank, and there's a friendly piece behind it
            // (behind = further from the enemy slider along that line).
            for pi in 0..all_n[ci] {
                ...
                for si in 0..slider_n[oci] {
                    ...
                    // Slider must be on same file or same rank
                    if sr != pr && sf != pf {
                        continue;
                    }

                    // Determine direction from slider to piece
                    let dr: i32 = if sr == pr { 0 } else if pr > sr { 1 } else { -1 };
                    let df: i32 = if sf == pf { 0 } else if pf > sf { 1 } else { -1 };

                    // Check that the path from slider to piece is clear
                    let mut clear = true;
                    let mut cr = sr + dr;
                    let mut cf = sf + df;
                    while (cr, cf) != (pr, pf) {
                    ...
                    // Now check if there's a friendly piece behind this piece
                    // (further along the same direction from slider)
                    let br = pr + dr;
                    let bf = pf + df;
        ...
        for i in 0..ml.moves.len() {
            ...
            if i >= 3 && depth >= 2 && !is_tactical && !gives_check && !in_check {
                ...
                if s0 > alpha && !self.stopped {
                    self.lmr_re += 1;
                    // Re-search with full window if PV, otherwise full depth zero-window
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList, stats: &mut SearchStats) {
        ...
        let mut diag = format!(
            r#"{{"engine":"wraith_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
...
impl Engine for WraithEngine {
    ...
    fn choose_move(&mut self, pos: &mut Position, budget: &SearchBudget) -> (Move, SearchStats) {
        ...
        for depth in 1..=30 {
            ...
            // Do the root search inline: iterate over root moves, track best
            // directly. Never rely on TT for the root best move.
            let mut iter_best;

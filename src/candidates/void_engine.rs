// void_002: Pure Search — zero heuristics, maximum depth, maximum NPS
//
// Thesis: material + PST only eval. Chronos search architecture.
// Every cycle wrung out:
...
// - No delta pruning in qsearch (eval is free, search everything)
...
const PAWN_V: [i32; 64] = [100,100,100,100,100,100,100,100,105,110,110,80,80,110,110,105,105,95,90,100,100,90,95,105,100,100,100,120,120,100,100,100,105,105,110,125,125,110,105,105,110,110,120,130,130,120,110,110,150,150,150,150,150,150,150,150,100,100,100,100,100,100,100,100];
const KNIGHT_V: [i32; 64] = [270,280,290,290,290,290,280,270,280,300,320,325,325,320,300,280,290,325,330,335,335,330,325,290,290,320,335,340,340,335,320,290,290,325,335,340,340,335,325,290,290,320,330,335,335,330,320,290,280,300,320,320,320,320,300,280,270,280,290,290,290,290,280,270];
const BISHOP_V: [i32; 64] = [310,320,320,320,320,320,320,310,320,335,330,330,330,330,335,320,320,340,340,340,340,340,340,320,320,330,340,340,340,340,330,320,320,335,335,340,340,335,335,320,320,330,335,340,340,335,330,320,320,330,330,330,330,330,330,320,310,320,320,320,320,320,320,310];
const ROOK_V: [i32; 64] = [500,500,500,505,505,500,500,500,495,500,500,500,500,500,500,495,495,500,500,500,500,500,500,495,495,500,500,500,500,500,500,495,495,500,500,500,500,500,500,495,495,500,500,500,500,500,500,495,505,510,510,510,510,510,510,505,500,500,500,500,500,500,500,500];
const QUEEN_V: [i32; 64] = [880,890,890,895,895,890,890,880,890,900,900,900,900,900,900,890,890,900,905,905,905,905,900,890,895,900,905,905,905,905,900,895,900,900,905,905,905,905,900,895,890,905,905,905,905,905,900,890,890,900,905,900,900,900,900,890,880,890,890,895,895,890,890,880];
const KING_V: [i32; 64] = [20,30,10,0,0,10,30,20,20,20,0,0,0,0,20,20,-10,-20,-20,-20,-20,-20,-20,-10,-20,-30,-30,-40,-40,-30,-30,-20,-30,-40,-40,-50,-50,-40,-40,-30,-30,-40,-40,-50,-50,-40,-40,-30,-30,-40,-40,-50,-50,-40,-40,-30,-30,-40,-40,-50,-50,-40,-40,-30];

// Flat lookup: EVAL_LUT[piece_type as usize * 64 + sq] for white
// For black: EVAL_LUT[piece_type as usize * 64 + (sq ^ 56)]
static EVAL_LUT: LazyLock<[i32; 7 * 64]> = LazyLock::new(|| {
    let mut t = [0i32; 7 * 64];
    // piece_type 0 = None, leave as 0
    for sq in 0..64 { t[1 * 64 + sq] = PAWN_V[sq]; }   // Pawn
    for sq in 0..64 { t[2 * 64 + sq] = KNIGHT_V[sq]; }  // Knight
    for sq in 0..64 { t[3 * 64 + sq] = BISHOP_V[sq]; }  // Bishop
    for sq in 0..64 { t[4 * 64 + sq] = ROOK_V[sq]; }    // Rook
    for sq in 0..64 { t[5 * 64 + sq] = QUEEN_V[sq]; }   // Queen
    for sq in 0..64 { t[6 * 64 + sq] = KING_V[sq]; }    // King
    t
});
...
fn eval_piece(pt: PieceType, c: Color, sq: u8) -> i32 {
    ...
    unsafe { *EVAL_LUT.get_unchecked(pt as usize * 64 + idx) }
}
...
impl Default for TTEntry {
    fn default() -> Self { Self { key32: 0, packed_move: 0, score: 0, depth: 0, flag: 0 } }
}

#[inline(always)]
fn pack_move(m: &Move) -> u32 {
    (m.from as u32) | ((m.to as u32) << 6) | ((m.path_kind as u32) << 12) |
    ((m.stop_index as u32) << 14) | ((m.special as u32) << 18) | ((m.promo_piece as u32) << 20)
}
...
fn unpack_move(p: u32) -> Move {
    Move {
        ...
        special: match (p >> 18) & 0x3 { 1 => SpecialMove::Castle, 2 => SpecialMove::EnPassant, 3 => SpecialMove::Promotion, _ => SpecialMove::None },
        promo_piece: match (p >> 20) & 0x7 { 1 => PieceType::Pawn, 2 => PieceType::Knight, 3 => PieceType::Bishop, 4 => PieceType::Rook, 5 => PieceType::Queen, 6 => PieceType::King, _ => PieceType::None },
    }
}
...
#[inline(always)] fn hist_idx(c: usize, f: usize, t: usize) -> usize { (c << 12) | (f << 6) | t }
...
impl MoveList {
    ...
    #[inline(always)]
    fn push(&mut self, m: Move) { unsafe { *self.moves.get_unchecked_mut(self.len) = m; } self.len += 1; }
    // Lazy sort: find best from position i onward, swap to i
    #[inline(always)]
    fn pick_best(&mut self, from: usize) {
        let mut bi = from;
        let mut bv = self.scores[from];
        for j in (from + 1)..self.len {
            let v = unsafe { *self.scores.get_unchecked(j) };
            if v > bv { bv = v; bi = j; }
        }
        if bi != from { self.moves.swap(from, bi); self.scores.swap(from, bi); }
    }
}
...
impl VoidEngine {
    fn new() -> Self {
        ...
        Self {
            ...
            null_cuts: 0, lmr_tries: 0, lmr_re: 0, max_ply: 0,
            stopped: false, budget_max_time_us: 0, t0: Instant::now(),
            cut_hist: [0; 6], cut_types: [0; 3], max_qdep: 0,
        }
    }
    ...
    fn check_time(&mut self) -> bool {
        ...
        if self.budget_max_time_us > 0 && (self.nodes & 511) == 0 {
    ...
    fn tt_store(&mut self, key: u64, depth: i32, score: i32, flag: u8, m: &Move) {
        ...
        let e = unsafe { self.tt.get_unchecked_mut(idx) };
    ...
    fn evaluate(&self, pos: &Position) -> i32 {
        ...
        let board = &pos.board;
        for sq in 0u8..64 {
            let p = unsafe { *board.get_unchecked(sq as usize) };
            if p.piece_type != PieceType::None {
                let v = eval_piece(p.piece_type, p.color, sq);
                if p.color == stm { score += v; } else { score -= v; }
            }
        }
        score
    }
    ...
    fn score_moves(&self, pos: &Position, ml: &mut MoveList, ply: usize, ttm: &Move) {
        ...
        for i in 0..ml.len {
            let m = unsafe { ml.moves.get_unchecked(i) };
            ...
            if !tp.is_empty() {
                if tp.color == stm {
                    ...
                    sv = 1_000_000 + pval(tp.piece_type) * 10
                        + unsafe { *self.push_history.get_unchecked(hist_idx(stm as usize, m.from as usize, m.to as usize)) } as i32;
                } else {
                ...
                if ply < 64 && *m == self.killers[ply][0] { sv = 500_000; }
                    ...
                    sv = unsafe { *self.history.get_unchecked(hist_idx(stm as usize, m.from as usize, m.to as usize)) } as i32
    ...
    fn search(&mut self, pos: &mut Position, depth: i32, mut alpha: i32, mut beta: i32, ply: i32, in_check: bool, is_pv: bool) -> i32 {
        ...
        let mut ttm = Move::default();
        {
            ...
            if e.key32 == k32 {
                ...
                if e.depth as i32 >= depth && !is_pv {
                    ...
                    if e.flag == 0 { return s; }
                    if e.flag == 2 && s >= beta { return s; }
                    if e.flag == 1 && s <= alpha { return s; }
                }
            }
        }
        ...
        if !is_pv && !in_check && ply > 0 && depth >= 3 {
            ...
            let se = pos.ep_square; let ss = pos.side_to_move; let sz = pos.zobrist;
        ...
        for i in 0..ml.len {
            ...
            let push_hist = if is_push { unsafe { *self.push_history.get_unchecked(hist_idx(stm as usize, m.from as usize, m.to as usize)) } } else { 0 };
            let is_tactical = is_capture || is_promo || m.special == SpecialMove::EnPassant || (is_push && push_hist >= 0);

            if !is_tactical && !in_check && search_depth <= 2 && i as i32 >= 8 + search_depth * 4 { continue; }
            ...
            if i >= 3 && search_depth >= 2 && !is_tactical && !gives_check && !in_check {
                ...
                let hs = unsafe { *self.history.get_unchecked(hist_idx(ci, m.from as usize, m.to as usize)) } as i32;
                if hs < -500 { r += 2; } else if hs < -100 { r += 1; }
                r = r.clamp(1, search_depth - 1);
                let s0 = -self.search(pos, search_depth - 1 - r, -(alpha + 1), -alpha, ply + 1, gives_check, false);
                if s0 > alpha && !self.stopped {
                    self.lmr_re += 1;
                    score = -self.search(pos, search_depth - 1, -beta, -alpha, ply + 1, gives_check, is_pv);
                } else { score = s0; }
            } else {
                let ext = if gives_check && search_depth <= 4 { 1 } else { 0 };
                if i > 0 && !self.stopped {
                    let s1 = -self.search(pos, search_depth - 1 + ext, -(alpha + 1), -alpha, ply + 1, gives_check, false);
                    if s1 > alpha && s1 < beta && !self.stopped {
                        score = -self.search(pos, search_depth - 1 + ext, -beta, -alpha, ply + 1, gives_check, is_pv);
                    } else { score = s1; }
                } else {
                    score = -self.search(pos, search_depth - 1 + ext, -beta, -alpha, ply + 1, gives_check, is_pv);
                }
            ...
            if alpha >= beta {
                ...
                let ct = if is_capture { 0 } else if is_push { 1 } else { 2 };
                self.cut_types[ct] += 1;
                let hi = if i < 5 { i } else { 5 };
                self.cut_hist[hi] += 1;
                let ci = pos.side_to_move as usize;
                if is_push {
                    let h = unsafe { self.push_history.get_unchecked_mut(hist_idx(ci, m.from as usize, m.to as usize)) };
                    *h = (*h + (depth * depth) as i16).min(16000);
                    for j in 0..i {
                        let pm = ml.moves[j]; let pt2 = pos.board[pm.to as usize];
                        if !pt2.is_empty() && pt2.color == stm {
                            let hh = unsafe { self.push_history.get_unchecked_mut(hist_idx(ci, pm.from as usize, pm.to as usize)) };
                    ...
                    let h = unsafe { self.history.get_unchecked_mut(hist_idx(ci, m.from as usize, m.to as usize)) };
                    *h = (*h + (depth * depth) as i16).min(16000);
                    for j in 0..i {
                        let pm = ml.moves[j]; let pt2 = pos.board[pm.to as usize];
                        let pc = !pt2.is_empty() && pt2.color != stm;
                        let pp = !pt2.is_empty() && pt2.color == stm;
                        if !pc && !pp && pm.special != SpecialMove::Promotion && pm.special != SpecialMove::EnPassant {
                            let hh = unsafe { self.history.get_unchecked_mut(hist_idx(ci, pm.from as usize, pm.to as usize)) };
                            *hh = (*hh - depth as i16).max(-16000);
                        }
                    }
    ...
    fn qsearch(&mut self, pos: &mut Position, mut alpha: i32, beta: i32, qdepth: i32) -> i32 {
        ...
        if qdepth >= 10 { return self.evaluate(pos); }
        ...
        for m in &buf {
            ...
            let t = ic || m.special == SpecialMove::Promotion || m.special == SpecialMove::EnPassant;
            if !t { continue; }
            let mut see = pval(pos.board[m.to as usize].piece_type);
            if m.special == SpecialMove::Promotion { see += 800; }
            if sp + see + 200 < alpha { continue; } // SEE delta pruning still kept (cheap)
            if nt < 64 { tac[nt] = (*m, see); nt += 1; }
        }
        self.move_buf = buf;

        // Inline selection sort for qsearch tactical moves
        for i in 0..nt {
            let mut bi = i; let mut bv = tac[i].1;
            for j in (i + 1)..nt { if tac[j].1 > bv { bv = tac[j].1; bi = j; } }
            if bi != i { tac.swap(i, bi); }
        }
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList, stats: &mut SearchStats) {
        ...
        for i in 0..root.len {
            ...
            pos.make_move(m);
            let key = pos.zobrist;
            let e = unsafe { self.tt.get_unchecked((key as usize) & TT_MASK) };
            ...
            if m.special == SpecialMove::Promotion { match m.promo_piece { PieceType::Knight => uci.push('n'), PieceType::Bishop => uci.push('b'), PieceType::Rook => uci.push('r'), PieceType::Queen => uci.push('q'), _ => {} } }
            ranked.push((uci, sc));
        }
        ...
        let mut diag = format!(
            ...
            self.qnodes, self.tt_hits, self.beta_cuts, self.first_cuts, self.null_cuts,
            self.lmr_tries, self.lmr_re, self.max_qdep, hf,
            self.cut_hist[0], self.cut_hist[1], self.cut_hist[2], self.cut_hist[3], self.cut_hist[4], self.cut_hist[5],
            self.cut_types[0], self.cut_types[1], self.cut_types[2]);
...
impl Engine for VoidEngine {
    ...
    fn choose_move(&mut self, pos: &mut Position, budget: &SearchBudget) -> (Move, SearchStats) {
        ...
        self.prev_move = Move::default();
        self.cut_hist = [0; 6]; self.cut_types = [0; 3]; self.max_qdep = 0;
        ...
        for depth in 1..=30 {
            ...
            loop {
                ...
                for i in 0..root.len {
                    if root.moves[i] == best { if i != 0 { root.moves.swap(0, i); } break; }
                }
                ...
                if iter_best_score <= (best_score - 50) && !af {
                    alpha = (iter_best_score - 200).max(-100000); beta = 100000; af = true; continue;
                }
                if iter_best_score >= (best_score + 50) && depth > 3 && best_score.abs() <= 5000 && !af {
                    alpha = -100000; beta = (iter_best_score + 200).min(100000); af = true; continue;
                }
                break;
            }
        ...
        // PV extraction
        stats.pv.clear();
        let mut seen = [0u64; 32]; let mut sn = 0; let mut dt = 0;
        loop {
            if dt >= 32 { break; }
            let key = pos.zobrist;
...
pub fn create() -> Box<dyn Engine> { Box::new(VoidEngine::new()) }

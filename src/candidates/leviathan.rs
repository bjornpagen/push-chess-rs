...
impl LeviathanEngine {
    ...
    fn lazy_evaluate(&self, pos: &Position) -> i32 {
        ...
        for sq in 0..64 {
            let p = pos.board[sq];
            if !p.is_empty() {
                s += (if p.color == stm { 1 } else { -1 })
                    * (pval(p.piece_type) + pst_value(p.piece_type, p.color, sq as u8));
            }
        }
        s
    }
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList, stats: &mut SearchStats) {
        ...
        let mut diag = format!(
            r#"{{"engine":"leviathan_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"max_qdep":{},"hashfull":{},"cut_hist":[{},{},{},{},{},{}],"cut_types":{{"cap":{},"push":{},"quiet":{}}},"top_moves":["#,
            self.qnodes,
            self.tt_hits,
            self.beta_cuts,
            self.first_cuts,

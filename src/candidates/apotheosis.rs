...
impl ApotheosisEngine {
    ...
    fn evaluate(&self, pos: &Position) -> i32 {
        ...
        for sq in 0u8..64 {
            ...
            } else if pt == PieceType::Rook || pt == PieceType::Queen {
                if c == Color::White {
                    w_rf |= 1 << f;
                    w_rr |= 1 << r;
                } else {
        ...
        for sq in 0u8..64 {
            ...
            if pt == PieceType::Pawn {
                ...
                score += sign * (adv * adv * 3);
            }
        }

        // Tensor king safety
        let wk_d1 = (wk_r - wk_f + 7) as usize;
        let wk_d2 = (wk_r + wk_f) as usize;
        let bk_d1 = (bk_r - bk_f + 7) as usize;
        let bk_d2 = (bk_r + bk_f) as usize;
        if (w_rf & (1 << bk_f)) != 0
            || (w_rr & (1 << bk_r)) != 0
            || (w_d1 & (1 << bk_d1)) != 0
            || (w_d2 & (1 << bk_d2)) != 0
        {
            score += if stm == Color::White { 40 } else { -40 };
        }
        if (b_rf & (1 << wk_f)) != 0
            || (b_rr & (1 << wk_r)) != 0
            || (b_d1 & (1 << wk_d1)) != 0
        ...
        for i in 0..ml.len() {
            ...
            if !pos.board[m.to as usize].is_empty() {
                if pos.board[m.to as usize].color == stm {
                    sv += 90_000 + pval(pos.board[m.to as usize].piece_type);
                } else {
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList, stats: &mut SearchStats) {
        ...
        let mut diag = format!(
            r#"{{"engine":"apotheosis_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lazy":{},"lmr":[{},{}],"top_moves":["#,
            self.qnodes,
            self.tt_hits,
            self.beta_cuts,
            self.first_cuts,

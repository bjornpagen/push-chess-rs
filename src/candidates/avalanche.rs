// avalanche_021: King safety focus for black-dominant meta (68.5% black wins)
//
// Enhanced chimera eval with doubled king safety penalties, king zone attack
// scoring (nonlinear attacker^2 penalty), and bishop pair bonus.
// Search: colossus's aggressive LMR/1.5 + tempest's is_pv tracking.
// Wider LMP, deeper futility, deeper check extensions.
//
// Key params:
//   - Exposed king penalty: -80 (doubled from -40)
//   - Open file penalty: -50 (doubled from -25)
//   - Pawn shield: +25 per shield pawn (was +15)
//   - King zone attack: attackers^2 * 4 nonlinear penalty
//   - Bishop pair: +40 (was +30)
//   - LMP: depth <= 3, prune at 6 + depth*3
//   - Futility: depth <= 5, 120cp/ply
...
// -----------------------------------------------------------------------
// LMR table — divisor 1.5 (from colossus)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// MoveList (stack-allocated, fixed-size)
// -----------------------------------------------------------------------
...
impl Avalanche {
    ...
    // Enhanced chimera eval with significantly boosted king safety
    fn evaluate(&self, pos: &Position) -> i32 {
        ...
        // Track non-pawn, non-king pieces for king zone attack scoring
        ...
        // Piston bonus
        for ci in 0..2usize {
            let c: Color = if ci == 0 { Color::White } else { Color::Black };
            let sign: i32 = if c == stm { 1 } else { -1 };
        ...
        for ci in 0..2usize {
            ...
            for pi in 0..pawn_n[oci] {
                ...
                let pr = psq >> 3;
                let pf = psq & 7;
                let oc: Color = if oci == 0 { Color::White } else { Color::Black };
        ...
        // Enhanced king safety (doubled penalties for king push meta)
        for ci in 0..2usize {
            let c: Color = if ci == 0 { Color::White } else { Color::Black };
            let sign: i32 = if c == stm { 1 } else { -1 };
            let kr = (king[ci] >> 3) as i32;
            ...
            // Pawn shield bonus: +25 per shield pawn (was +15)
            for pi in 0..pawn_n[ci] {
                let psq = pawn_sq[ci][pi] as i32;
                let pr = psq >> 3;
                let pf = psq & 7;
            ...
            // Exposed king penalty doubled: -80 if <2 pawns near king (was -40)
            ...
            // Open file penalty doubled: -50 if no pawn on king's file (was -25)
            ...
            // Count enemy pieces within Chebyshev distance 2 of king,
            // apply nonlinear penalty: attackers^2 * 4
            let oci = 1 - ci;
            let mut attackers_near_king = 0i32;
            ...
            if attackers_near_king > 0 {
                sc_pos += sign * (-(attackers_near_king * attackers_near_king * 4));
            }
        }
        ...
        for i in 0..ml.moves.len() {
            ...
            if *m == *ttm {
                ...
                if m.special == SpecialMove::Promotion {
                    sc += 95000.0 + pval(m.promo_piece) as f32;
                }
            ...
            if alpha >= beta {
                ...
                if !is_tactical && (ply as usize) < 64 {
                    ...
                    *h = (*h + (depth * depth) as i16).min(16000);
                    for j in 0..i {
                        let mj = ml.moves[j];
                        if pos.board[mj.to as usize].is_empty() {
                            let hh = &mut self.history[ci][mj.from as usize][mj.to as usize];
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList) -> String {
        let mut ranked: Vec<(String, i32)> = Vec::with_capacity(root.moves.len());
        for i in 0..root.moves.len() {
            ...
            let mut uci = String::with_capacity(6);
        ...
        let mut out = format!(
            r#"{{"engine":"avalanche_021","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
        ...
        for i in 0..cap {
            ...
            out.push_str(&format!(r#"["{}",{}]"#, ranked[i].0, ranked[i].1));
        }
        out.push_str("]}");
        out
    }
...
impl Engine for Avalanche {
    ...
    fn choose_move(
        ...
        // Validate returned move against legal list
        let mut valid = false;
        for i in 0..root.moves.len() {

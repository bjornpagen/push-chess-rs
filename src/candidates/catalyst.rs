// catalyst_001: Improving flag + extended futility specialist
//
// Based on phantom_018 (megabrawl champion 78.9%).
// Innovation 1: Improving flag — tracks static eval at each ply, adjusts
//   reverse futility margins, LMP thresholds, and LMR reductions.
// Innovation 2: Extended futility to depth 6 (up from depth 4).
...
impl Catalyst {
    ...
    fn evaluate(&self, pos: &Position) -> i32 {
        ...
        for ci in 0..2usize {
            ...
            if pawns_near_king < 2 { sc_pos += sign * (-50); }

            let mut has_pawn_on_file = false;
            for pi in 0..pawn_n[ci] {
                if (pawn_sq[ci][pi] & 7) as i32 == kf { has_pawn_on_file = true; break; }
            }
            if !has_pawn_on_file { sc_pos += sign * (-30); }

            if bishop_count[ci] >= 2 { sc_pos += sign * 40; }
        }
        ...
        for ci in 0..2usize {
            ...
            for si in 0..slider_n[ci] {
                ...
                if !friendly_pawn && !enemy_pawn { sc_pos += sign * 25; }
                else if !friendly_pawn { sc_pos += sign * 15; }
    ...
    fn search(&mut self, pos: &mut Position, depth: i32, mut alpha: i32, mut beta: i32, ply: i32, in_check: bool) -> i32 {
        ...
        // === INNOVATION 1 & 2: Improving flag + extended futility ===
        // Compute static eval and store in eval_stack
        let eval = self.evaluate(pos);
        self.eval_stack[ply as usize] = eval;

        // Determine if position is improving (current eval > eval from 2 plies ago)
        let improving = ply >= 2 && self.eval_stack[ply as usize] > self.eval_stack[(ply - 2) as usize];

        // Reverse futility: extended to depth <= 6 (was depth <= 4)
        // Improving: tighter margin (depth * 80), not improving: wider margin (depth * 120)
        if !in_check && depth <= 6 && ply > 0 {
            let rfp_margin = if improving { depth * 80 } else { depth * 120 };
            if eval - rfp_margin >= beta { self.null_cuts += 1; return beta; }
        ...
        // LMP threshold adjusted by improving flag
        let lmp_threshold = if improving { 10 + depth * 5 } else { 6 + depth * 3 };
        ...
        while i < ml.moves.len() && !self.stopped {
            ...
            // LMP: adjusted by improving flag
            if !is_tactical && !in_check && depth <= 2 && i as i32 >= lmp_threshold {
                i += 1; continue;
            }
            ...
            if i >= 3 && depth >= 2 && !is_tactical && !gives_check && !in_check {
                ...
                // Innovation 1: when improving, reduce r by 1 (search deeper)
                if improving { r -= 1; }
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList) -> String {
        ...
        let mut s = format!(
            r#"{{"engine":"catalyst_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
...
impl Engine for Catalyst {
    fn name(&self) -> &str { "catalyst" }

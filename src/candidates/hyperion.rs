...
impl HyperionEngine {
    ...
    fn evaluate(&self, pos: &Position) -> i32 {
        ...
        let mut score = 0;
        let wk_sq = pos.king_sq[Color::White as usize];
        let bk_sq = pos.king_sq[Color::Black as usize];
        let wk_r = (wk_sq >> 3) as i32;
        let wk_f = (wk_sq & 7) as i32;
        let bk_r = (bk_sq >> 3) as i32;
        let bk_f = (bk_sq & 7) as i32;
        ...
        for sq in 0u8..64 {
            ...
            if pt == PieceType::Rook || pt == PieceType::Queen {
                let (ek_f, ek_r) = if c == Color::White {
                    (bk_f, bk_r)
                } else {
                    (wk_f, wk_r)
                };
    ...
    fn evaluate_with_terms(&self, pos: &Position) -> (i32, String) {
        ...
        let terms = format!(
            r#"{{"mat":{},"pst":{},"xray":{},"spear":{},"king":{}}}"#,
            mat, pst, xray, spear, king
        );
        (total, terms)
    }
    ...
    fn order_moves(&self, pos: &Position, ml: &mut MoveList, ply: usize, ttm: &Move) {
        ...
        for i in 0..ml.len() {
            ...
            if !tp.is_empty() {
                if tp.color == stm {
                    sv += 90_000
                        + if tp.piece_type == PieceType::Pawn {
                            5000
                        } else {
                            pval(tp.piece_type)
                        };
                    sv += self.history[history_idx(stm as usize, m.from as usize, m.to as usize)]
                        as i32;
                } else {
    ...
    fn search(
        ...
        for i in 0..ml.len() {
            ...
            if !is_tactical && !in_check && depth <= 4 && i as i32 >= 4 + depth * 2 {
                continue;
            }
            self.prev_move = m;
            pos.make_move(&m);
            ...
            if alpha >= beta {
                    ...
                    && m.special != SpecialMove::EnPassant
                {
                    ...
                    *h = (*h + (depth * depth) as i16).min(16000);
                    for j in 0..i {
                        let pm = ml[j].mv;
                        let pc = !pos.board[pm.to as usize].is_empty()
                            && pos.board[pm.to as usize].color != stm;
                        if !pc
    ...
    fn qsearch(&mut self, pos: &mut Position, mut alpha: i32, beta: i32, qdepth: i32) -> i32 {
        ...
        self.nodes += 1;
        self.qnodes += 1;
        let qd = qdepth as u32;
        if qd > self.max_qdepth {
            self.max_qdepth = qd;
        }
        if qdepth >= 4 {
            return self.evaluate(pos);
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList, stats: &mut SearchStats) {
        ...
        let mut diag = format!(
            r#"{{"engine":"hyperion_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
            self.qnodes,
            self.tt_hits,
            self.beta_cuts,
            self.first_cuts,

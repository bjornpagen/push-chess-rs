// vortex_001: Deep search via null move + aggressive pruning
//
// Based on specter_020 (tournament champion). Key changes:
//   - Null Move Pruning: R=3 for depth>=6, R=2 otherwise (NEW)
//   - PV-node awareness: is_pv parameter, skip NMP/RFP/TT-cut in PV nodes
//   - More aggressive LMP: depth <= 3, threshold 6 + depth*3
//   - Wider reverse futility: depth <= 5, 120cp/ply
//   - Larger TT: 1M entries (1 << 20)
//   - Everything else from specter: eval, direct best-move, NEON, LMR 1.8,
//     check extension, aspiration, killer/history/countermove
...
impl VortexEngine {
    ...
    // Chimera eval + king-push-awareness (identical to specter)
    ...
    fn search(
        ...
        // Skip in PV nodes, when in check, at root, or at shallow depth
        if !is_pv && !in_check && ply > 0 && depth >= 3 {
            ...
            // Make null move: flip side, clear ep, update zobrist
            ...
            // Unmake null move: restore everything
        ...
        // Reverse futility pruning (aggressive: depth <= 5, 120cp/ply)
        // Skip in PV nodes
        if !is_pv && !in_check && depth <= 5 && ply > 0 {
            let eval = self.evaluate(pos);
            if eval - depth * 120 >= beta {
                return beta;
            }
        ...
        for i in 0..ml.moves.len() {
            ...
            // Aggressive LMP: depth <= 3, prune at 6 + depth*3
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList, stats: &mut SearchStats) {
        ...
        let mut diag = format!(
            r#"{{"engine":"vortex_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
            self.qnodes,
            self.tt_hits,
            self.beta_cuts,
            self.first_cuts,

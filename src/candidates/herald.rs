// herald_001: Repetition detection in search
//
// Based on phantom_018 (megabrawl champion, 78.9%).
// Adds search-internal repetition detection to avoid/force draws.
// Phantom drew 6 of 19 games (31.6% draw rate, 3x tournament average).
// Herald detects repeated positions during search and returns draw score (0).
...
struct Herald {
    ...
    budget_time_us: i64,
    t0: Instant,
    // Repetition detection: zobrist keys encountered during search
    search_keys: Vec<u64>,
}
...
impl Herald {
    fn new() -> Self {
        ...
        Self {
            ...
            budget_time_us: 0,
            t0: Instant::now(),
            search_keys: Vec::with_capacity(128),
        }
    }
    ...
    fn search(&mut self, pos: &mut Position, depth: i32, mut alpha: i32, mut beta: i32, ply: i32, in_check: bool) -> i32 {
        ...
        // Repetition detection: if this position has occurred before in the search, return draw score
        if ply > 0 {
            let key = pos.zobrist;
            if self.search_keys.iter().rev().step_by(2).any(|&k| k == key) {
                return 0;
            }
        }
        ...
        while i < ml.moves.len() && !self.stopped {
            ...
            if i >= 3 && depth >= 2 && !is_tactical && !gives_check && !in_check {
                ...
                // Push position for repetition detection before recursing
                self.search_keys.push(pos.zobrist);
                score = -self.search(pos, depth - 1 - r, -(alpha+1), -alpha, ply + 1, gives_check);
                self.search_keys.pop();
    ...
    fn qsearch(&mut self, pos: &mut Position, mut alpha: i32, beta: i32, qdepth: i32) -> i32 {
        ...
        for i in 0..tactical.len() {
            ...
            // Push position for repetition detection in qsearch
            self.search_keys.push(pos.zobrist);
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList) -> String {
        ...
        let mut s = format!(
            r#"{{"engine":"herald_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
...
impl Engine for Herald {
    fn name(&self) -> &str { "herald" }
    ...
    fn choose_move(&mut self, pos: &mut Position, budget: &SearchBudget) -> (Move, SearchStats) {
        ...
        self.prev_move = Move::default();
        // Clear search keys for this new search
        self.search_keys.clear();

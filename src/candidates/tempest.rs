// tempest_016: First engine with PV-node TT cutoff restriction
//
// Base: chimera_013 (best Gen3, all bugs fixed)
// Added: is_pv parameter to search -- don't cut TT in PV nodes
// Kept: chimera's full eval (king safety, piston, tropism)
// TT: 512K
...
impl TempestEngine {
    ...
    fn search(
        ...
        if tt_key32 == k32 {
            ...
            // KEY DIFFERENCE: don't cut in PV nodes
            if tt_depth as i32 >= depth && !is_pv {
                self.tt_hits += 1;
                if tt_flag == 0 {
                    return tt_score;
                }
        ...
        while i < ml.moves.len() && !self.stopped {
            ...
            if i >= 3 && depth >= 2 && !is_tactical && !gives_check && !in_check {
                ...
                if s > alpha && !self.stopped {
                    self.lmr_re += 1;
                    s = -self.search(pos, depth - 1, -beta, -alpha, ply + 1, gives_check, is_pv);
                }
                score = s;
            } else {

// fortress_001: Eval Cache + IIR (Internal Iterative Reduction)
//
// Based on phantom_018 (megabrawl champion, 78.9%).
// Two pure speed optimizations for more search depth:
// 1. Evaluation cache (64K entries) to avoid redundant evaluate() calls
// 2. IIR replaces IID — just reduce depth by 1 when no TT move, instead of
//    performing a costly depth-2 search.
...
impl Fortress {
    ...
    fn cached_evaluate(&mut self, pos: &Position) -> i32 {
        let key = pos.zobrist;
        let idx = (key as usize) & EVAL_CACHE_MASK;
        let k32 = (key >> 32) as u32;
        if self.eval_cache[idx].key32 == k32 {
            return self.eval_cache[idx].score as i32;
        }
        let score = self.evaluate(pos);
        self.eval_cache[idx] = EvalCacheEntry { key32: k32, score: score as i16 };
        score
    }
    ...
    fn search(&mut self, pos: &mut Position, depth: i32, mut alpha: i32, mut beta: i32, ply: i32, in_check: bool) -> i32 {
        ...
        if ply >= 128 { return self.cached_evaluate(pos); }
        ...
        if !in_check && depth <= 4 && ply > 0 {
            let eval = self.cached_evaluate(pos);
        ...
        // IIR: reduce depth when no TT move available (replaces IID)
        let mut search_depth = depth;
        if move_is_null(&ttm) && depth >= 4 {
            search_depth -= 1;
        }
        ...
        while i < ml.moves.len() && !self.stopped {
            ...
            if i >= 3 && search_depth >= 2 && !is_tactical && !gives_check && !in_check {
                self.lmr_tries += 1;
                let mut r = LMR_TABLE[d][mi].clamp(1, search_depth - 1);
                ...
                score = -self.search(pos, search_depth - 1 - r, -(alpha+1), -alpha, ply + 1, gives_check);
                if score > alpha && !self.stopped {
                    self.lmr_re += 1;
                    let s2 = -self.search(pos, search_depth - 1, -beta, -alpha, ply + 1, gives_check);
                    pos.unmake_move();
                ...
                if i > 0 && !self.stopped {
                    score = -self.search(pos, search_depth - 1 + ext, -(alpha+1), -alpha, ply + 1, gives_check);
                    if score > alpha && score < beta && !self.stopped {
                        let s2 = -self.search(pos, search_depth - 1 + ext, -beta, -alpha, ply + 1, gives_check);
                        pos.unmake_move();
                        if s2 > best_score { best_score = s2; best_move = m; }
                        if s2 > alpha { alpha = s2; flag = 0; }
                        if alpha >= beta {
                    ...
                    score = -self.search(pos, search_depth - 1 + ext, -beta, -alpha, ply + 1, gives_check);
                }
            }
            pos.unmake_move();
    ...
    fn qsearch(&mut self, pos: &mut Position, mut alpha: i32, beta: i32, qdepth: i32) -> i32 {
        ...
        if qdepth >= 4 { return self.cached_evaluate(pos); }

        let stand_pat = self.cached_evaluate(pos);
    ...
    fn dump_diag(&mut self, pos: &mut Position, root: &MoveList) -> String {
        ...
        let mut s = format!(
            r#"{{"engine":"fortress_001","qn":{},"tt":{},"bcut":{},"fcut":{},"nmp":{},"lmr":[{},{}],"top_moves":["#,
    ...
    fn choose_move(&mut self, pos: &mut Position, budget: &SearchBudget) -> (Move, SearchStats) {
        ...
        if best_score <= -90000 {
            ...
            for i in 0..root.moves.len() {
                ...
                let sc = -self.cached_evaluate(pos);
                pos.unmake_move();
                if sc > bs { bs = sc; best = root.moves[i]; }
            }
        }

        // Validate chosen move against legal moves
        let mut valid = false;
        for i in 0..root.moves.len() {

...
use push_chess::core::types::{self as chess, Move, PieceType, SpecialMove, SearchBudget, SearchStats};
...
fn parse_board(fen: &str) -> [[char; 8]; 8] {
    ...
    for c in fen.chars() {
        if c == ' ' { break; }
        if c == '/' { rank -= 1; file = 0; }
        else if c.is_ascii_digit() { file += (c as usize) - ('0' as usize); }
        else if rank >= 0 && rank < 8 && file < 8 { board[rank as usize][file] = c; file += 1; }
    }
    board
}
...
impl App {
    ...
    fn is_player_turn(&self) -> bool {
        !self.game_over && !self.engine_thinking && (
    ...
    fn start_engine_think(&mut self) {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                engine.new_game(
                    if player_is_white { chess::Color::Black } else { chess::Color::White },
                    42,
                );
                ...
                let piece_name = format!("{:?}", pos_clone.board[chosen.from as usize].piece_type);

                let _ = tx.send(EngineResult { chosen, stats, piece_name });
            })
    ...
    fn check_engine_result(&mut self) {
        ...
        if let Some(result) = got {
            ...
            self.move_history.push(MoveRecord {
                ...
                side: if self.player_is_white { "black" } else { "white" },
                eval: result.stats.eval_cp,
                depth: result.stats.depth_reached,
            });
            self.move_list_state.select(Some(self.move_history.len() - 1));
    ...
    fn try_move(&mut self, from: u8, to: u8) -> bool {
        let matches: Vec<&Move> = self.legal_moves.iter()
            .filter(|m| m.from == from && m.to == to)
            .collect();
        if matches.is_empty() { return false; }

        let chosen = if matches.len() > 1 {
            *matches.iter().find(|m| m.promo_piece == PieceType::Queen)
                .unwrap_or(&matches[0])
        } else {
            matches[0]
        };
        ...
        self.move_history.push(MoveRecord {
            uci: move_uci(&chosen),
            side: if self.player_is_white { "white" } else { "black" },
            eval: 0,
            depth: 0,
        });
        self.move_list_state.select(Some(self.move_history.len() - 1));
    ...
    fn handle_key(&mut self, code: KeyCode) -> bool {
        match code {
            ...
            KeyCode::Up | KeyCode::Char('k') => { if self.cursor_sq / 8 < 7 { self.cursor_sq += 8; } }
            KeyCode::Down | KeyCode::Char('j') => { if self.cursor_sq / 8 > 0 { self.cursor_sq -= 8; } }
            KeyCode::Left | KeyCode::Char('h') => { if self.cursor_sq % 8 > 0 { self.cursor_sq -= 1; } }
            KeyCode::Right | KeyCode::Char('l') => { if self.cursor_sq % 8 < 7 { self.cursor_sq += 1; } }
            KeyCode::Enter | KeyCode::Char(' ') => {
                if !self.is_player_turn() || self.game_over { return false; }
    ...
    fn draw(&mut self, frame: &mut Frame) {
        ...
        let you = if self.player_is_white { "White" } else { "Black" };
        let header = Paragraph::new(Line::from(vec![
            Span::styled(" Play vs ", Style::default().fg(Color::White)),
            Span::styled(&self.engine_name, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            Span::styled(format!("  (you are {}, {}ms/move)", you, self.think_time_ms), Style::default().fg(Color::DarkGray)),
        ...
        let board_block = Block::default().borders(Borders::ALL).border_style(Style::default().fg(Color::DarkGray)).title(" Board ");
        let board_inner = board_block.inner(main[0]);
        frame.render_widget(board_block, main[0]);

        let fen = self.pos.to_fen();
        render_board_with_highlights(&fen, self.cursor_sq, self.selected_from, &self.legal_moves, board_inner, frame);
        ...
        let items: Vec<ListItem> = self.move_history.iter().enumerate().map(|(i, m)| {
            let movenum = i / 2 + 1;
            let dot = if m.side == "white" { "." } else { "..." };
            let side_color = if m.side == "white" { Color::Cyan } else { Color::Red };
            let mut spans = vec![
                Span::styled(format!("{:>3}{:<3} ", movenum, dot), Style::default().fg(Color::DarkGray)),
                Span::styled(format!("{:<6}", m.uci), Style::default().fg(side_color)),
            ];
            if m.depth > 0 {
                spans.push(Span::styled(format!(" d{} {}cp", m.depth, m.eval), Style::default().fg(Color::DarkGray)));
            }
        ...
        let turn = if self.game_over {
            ...
            format!("Your turn ({})", if self.pos.side_to_move == chess::Color::White { "white" } else { "black" })
        } else {
            "Waiting...".to_string()
        };
        ...
        let help = Paragraph::new(Line::from(vec![
            " ←→↑↓".dark_gray(), " move cursor ".dark_gray(),
            " Enter".dark_gray(), " select/move ".dark_gray(),
            " q".dark_gray(), " quit".dark_gray(),
        ]));
        frame.render_widget(help, outer[2]);
    }
}

fn render_board_with_highlights(fen: &str, cursor: u8, selected: Option<u8>, legal: &[Move], area: Rect, frame: &mut Frame) {
    ...
    let targets: Vec<u8> = if let Some(from) = selected {
        legal.iter().filter(|m| m.from == from).map(|m| m.to).collect()
    } else { Vec::new() };

    let mut lines: Vec<Line> = Vec::new();
    for r in (0..8).rev() {
        let mut spans: Vec<Span> = Vec::new();
        spans.push(Span::styled(format!("  {} ", r + 1), Style::default().fg(Color::DarkGray)));
        for f in 0..8 {
            let sq = (r * 8 + f) as u8;
            let is_dark = (r + f) % 2 == 0;
            let bg = if selected == Some(sq) { selected_color }
                else if sq == cursor { cursor_color }
                else if targets.contains(&sq) { target_color }
                else if is_dark { dark } else { light };
            let p = board[r][f];
            if p == '.' {
                let dot = if targets.contains(&sq) { " · " } else { "   " };
                spans.push(Span::styled(dot, Style::default().fg(Color::DarkGray).bg(bg)));
            } else {
                let fg = if p.is_ascii_uppercase() { white_piece } else { black_piece };
                spans.push(Span::styled(format!(" {} ", piece_char(p)), Style::default().fg(fg).bg(bg)));
            }
        }
        lines.push(Line::from(spans));
    }
    lines.push(Line::from(Span::styled("     a  b  c  d  e  f  g  h", Style::default().fg(Color::DarkGray))));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if find_engine(engine_name).is_none() {
        ...
        eprintln!("Available: {:?}", push_chess::candidates::ENGINE_REGISTRY.iter().map(|e| e.name).collect::<Vec<_>>());
        std::process::exit(1);
    }

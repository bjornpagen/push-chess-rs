...
fn main() {
    ...
    if args.len() < 2 || args[1] == "list" || args[1] == "-l" {
        let db_path = if args.len() >= 2 && args[1] != "list" && args[1] != "-l" { &args[1] } else { "pushchess.db" };
        ...
        let rows = stmt.query_map([], |r| Ok((
            r.get::<_,i32>(0)?, r.get::<_,String>(1)?, r.get::<_,i64>(2)?,
            r.get::<_,i32>(3)?, r.get::<_,String>(4)?, r.get::<_,i32>(5)?,
        ))).unwrap();
        eprintln!("Tournaments in {}:", db_path);
        eprintln!("{:<4} {:<20} {:>8} {:>5} {:>6} {:>6}", "ID", "Name", "budget", "g/m", "games", "status");
        for r in rows {
            let (id, name, budget, gpm, status, games) = r.unwrap();
            eprintln!("{:<4} {:<20} {:>6}ms {:>5} {:>6} {:>6}", id, name, budget/1000, gpm, games, status);
        }
        ...
        let rows2 = stmt2.query_map([], |r| Ok((
            r.get::<_,i64>(0)?, r.get::<_,i64>(1)?, r.get::<_,i32>(2)?,
            r.get::<_,String>(3)?, r.get::<_,i32>(4)?,
        ))).unwrap();
        let mut has_matches = false;
        for r in rows2 {
            if !has_matches { eprintln!("\nStandalone matches:"); eprintln!("{:<6} {:<30} {:>8} {:>6}", "m:ID", "Engines", "budget", "games"); has_matches = true; }
            let (id, budget, _ng, engines, games) = r.unwrap();
            eprintln!("m:{:<4} {:<30} {:>6}ms {:>6}", id, engines, budget/1000, games);
        }
    ...
    enum Filter { Tournament(i32), Match(i64), All }

    let filter = if selector == "latest" {
        // Find the most recent match (tournament or standalone)
        let mid: i64 = conn.prepare("SELECT match_id FROM matches ORDER BY match_id DESC LIMIT 1")
            .unwrap().query_row([], |r| r.get(0)).expect("no matches");
        // Check if it belongs to a tournament
        let tid: Option<i32> = conn.prepare("SELECT tournament_id FROM matches WHERE match_id=?1")
            .unwrap().query_row([mid], |r| r.get(0)).ok().flatten();
        if let Some(t) = tid { Filter::Tournament(t) } else { Filter::Match(mid) }
    } else if selector == "all" {
        Filter::All
    } else if selector.starts_with("m:") {
        Filter::Match(selector[2..].parse().expect("invalid match id"))
    } else {
        Filter::Tournament(selector.parse().expect("invalid tournament id — use `dump list`"))
    };

    // Build the WHERE clause for games
    let (game_where, header) = match &filter {
        Filter::Tournament(tid) => (
            format!("g.match_id IN (SELECT match_id FROM matches WHERE tournament_id={})", tid),
            format!("TOURNAMENT {}", tid),
        ),
        Filter::Match(mid) => (
            format!("g.match_id={}", mid),
            format!("MATCH {}", mid),
        ),
        Filter::All => (
            "1=1".to_string(),
    ...
    if let Filter::Tournament(tournament_id) = &filter {
        ...
        if let Ok((name, budget, gpm, started, finished, status)) = stmt.query_row([tournament_id], |r| Ok((r.get::<_,String>(0)?, r.get::<_,i64>(1)?, r.get::<_,i32>(2)?, r.get::<_,String>(3)?, r.get::<_,String>(4)?, r.get::<_,String>(5)?))) {
            println!("=== {} : {} ===", header, name);
            println!("budget={}us  games_per_matchup={}  status={}  started={}  finished={}", budget, gpm, status, started, finished);
        }
    } else {
        println!("=== {} ===", header);
    }
    ...
    let mut engines: HashSet<String> = HashSet::new();
    {
        ...
        for r in rows { engines.insert(r.unwrap()); }
    }
    ...
    println!("\n=== STANDINGS ===");
    {
        ...
        let rows = stmt.query_map([], |r| {
            Ok((r.get::<_,String>(0)?, r.get::<_,i32>(1)?, r.get::<_,i32>(2)?, r.get::<_,i32>(3)?, r.get::<_,f64>(4)?))
        }).unwrap();
        for r in rows {
    ...
    println!("\n=== GAMES ===");
    {
        ...
        let rows = stmt.query_map([], |r| {
            Ok((r.get::<_,i32>(0)?, r.get::<_,String>(1)?, r.get::<_,String>(2)?,
                r.get::<_,String>(3)?, r.get::<_,String>(4)?, r.get::<_,i32>(5)?, r.get::<_,i64>(6)?))
        }).unwrap();
        for r in rows {
            let (gid, w, b, res, term, ply, ms) = r.unwrap();
            println!("g{:<4} {:<14} v {:<14} {:<7} {:<20} {:>3}p {:>5}ms", gid, w, b, res, term, ply, ms);
        }
    }
    ...
    println!("\n=== SEARCH STATS ===");
    {
        let sql = format!(
            ...
            WHERE {}
            GROUP BY mv.candidate_id ORDER BY AVG(s.depth) DESC", game_where);
        let mut stmt = conn.prepare(&sql).unwrap();
        let rows = stmt.query_map([], |r| {
            Ok((r.get::<_,String>(0)?, r.get::<_,f64>(1)?, r.get::<_,f64>(2)?,
                r.get::<_,f64>(3)?, r.get::<_,f64>(4)?, r.get::<_,f64>(5)?, r.get::<_,i32>(6)?))
        }).unwrap();
        println!("{:<16} {:>5} {:>5} {:>8} {:>7} {:>8} {:>6}", "engine", "depth", "sdep", "nodes", "eval", "time_us", "moves");
        for r in rows {
            let (e, d, sd, n, ev, t, mv) = r.unwrap();
            println!("{:<16} {:>5.1} {:>5.1} {:>8.0} {:>7.0} {:>8.0} {:>6}", e, d, sd, n, ev, t, mv);
        }
    }

...
impl<const V: usize> Search<V> {
    pub(super) fn siege(&mut self, b: &mut Board, max_depth: i32) -> Option<Vec<Move>> {
        let limits = Limits {
            ...
            deadline: if self.limits.micros == 0 {
                ...
                self.limits.micros / 12
            },
            nodes: if self.limits.limit == 0 {
                20_000
            } else {
                (self.limits.limit / 12).max(1)
            },
        };
        for depth in (1..=max_depth.min(11)).step_by(2) {
            ...
            if self.limits.nodes >= limits.nodes
                || self.limits.started.elapsed().as_micros() >= limits.deadline
            {
                break;
            }
        }
    ...
    fn prove(
        ...
        if ply >= MAX - 2
            || self.limits.nodes >= limits.nodes
            || self.limits.started.elapsed().as_micros() >= limits.deadline
            || self.tick(ply)
        {
            return None;

//! Incremental piece-square baseline and outcome-trained neural evaluation.
//! No database access, opponent identity, or memorized game moves.
use super::board::{Board, Residual};
use super::network::{SCORE_LIMIT, TEMPO};
use crate::core::types::*;

pub const VALUE: [i32; 7] = [0, 100, 305, 365, 550, 1050, 0];
pub const PHASE: [i32; 7] = [0, 0, 1, 1, 2, 4, 0];
type PieceSquareTables = [[[(i32, i32); 64]; 7]; 2];
const fn piece_square(c: usize, pt: usize, sq: usize) -> (i32, i32) {
    let r = if c == 0 { sq / 8 } else { 7 - sq / 8 } as i32;
    let f = (sq % 8) as i32;
    let center = (if f <= 3 { 3 - f } else { f - 4 }) + (if r <= 3 { 3 - r } else { r - 4 });
    let advanced = if r > 3 { r - 3 } else { 0 };
    let (mg, eg) = match pt {
        1 => (
            r * 3 + advanced.pow(2) * 4 - (f - 3).abs() * 2,
            r * 7 + advanced.pow(2) * 8,
        ),
        2 => (36 - center * 11, 25 - center * 8),
        3 => (26 - center * 6, 30 - center * 5),
        4 => (r * 3, 12 - center * 3),
        5 => (12 - center * 3, 20 - center * 4),
        // No castling fetish: safety comes from the actual enemy attack geometry.
        6 => (-r * 5, 40 - center * 12),
        _ => (0, 0),
    };
    (VALUE[pt] + mg, VALUE[pt] + eg)
}

// No one-time initialization guard in every make/unmake feature update.
const PST: PieceSquareTables = {
    let mut table = [[[(0, 0); 64]; 7]; 2];
    let mut c = 0;
    while c < 2 {
        let mut pt = 0;
        while pt < 7 {
            let mut sq = 0;
            while sq < 64 {
                table[c][pt][sq] = piece_square(c, pt, sq);
                sq += 1;
            }
            pt += 1;
        }
        c += 1;
    }
    table
};

pub fn piece_score(p: Piece, sq: u8) -> (i32, i32) {
    PST[p.color as usize][p.piece_type as usize][sq as usize]
}

#[cfg(test)]
#[test]
fn constant_table_matches_the_original_geometry_for_every_entry() {
    for (c, side) in PST.iter().enumerate() {
        for (pt, row) in side.iter().enumerate() {
            for (sq, &actual) in row.iter().enumerate() {
                let r = if c == 0 { sq / 8 } else { 7 - sq / 8 } as i32;
                let f = (sq % 8) as i32;
                let center = (f - 3).abs().min((f - 4).abs()) + (r - 3).abs().min((r - 4).abs());
                let (mg, eg) = match pt {
                    1 => (
                        r * 3 + (r - 3).max(0).pow(2) * 4 - (f - 3).abs() * 2,
                        r * 7 + (r - 3).max(0).pow(2) * 8,
                    ),
                    2 => (36 - center * 11, 25 - center * 8),
                    3 => (26 - center * 6, 30 - center * 5),
                    4 => (r * 3, 12 - center * 3),
                    5 => (12 - center * 3, 20 - center * 4),
                    6 => (-r * 5, 40 - center * 12),
                    _ => (0, 0),
                };
                assert_eq!(actual, (VALUE[pt] + mg, VALUE[pt] + eg));
            }
        }
    }
}

pub(super) fn baseline_white(b: &Board<impl Residual>) -> i32 {
    let phase = b.phase.min(24);
    let baseline = ((b.mg[0] - b.mg[1]) * phase + (b.eg[0] - b.eg[1]) * (24 - phase)) / 24;
    let mut white = baseline;
    for c in 0..2 {
        let sign = if c == 0 { 1 } else { -1 };
        let mut pawns = b.men[c][1];
        while pawns != 0 {
            let sq = pawns.trailing_zeros() as usize;
            pawns &= pawns - 1;
            let rank = if c == 0 { sq / 8 } else { 7 - sq / 8 };
            if b.men[1 - c][1] & PASSED[c][sq] == 0 {
                white += sign * [0, 0, 5, 12, 24, 50, 100, 0][rank] * (32 - phase) / 16;
            }
        }
        if b.men[c][3].count_ones() >= 2 {
            white += sign * 30;
        }
    }
    white
}

pub fn evaluate<const V: usize>(b: &Board<impl Residual>) -> i32 {
    let profile = super::experiments::PROFILES[V];
    let mut white = baseline_white(b) + profile.neural_scale * b.net.white_residual();
    if profile.geometry {
        white += super::experiments::geometry(b);
    }
    if profile.logistics {
        white += super::experiments::logistics(b);
    }
    relative_score(white, b.pos.side_to_move)
}

pub(super) fn relative_score(white: i32, side: Color) -> i32 {
    (if side == Color::White { white } else { -white } + TEMPO).clamp(-SCORE_LIMIT, SCORE_LIMIT)
}

const PASSED: [[u64; 64]; 2] = {
    let mut result = [[0; 64]; 2];
    let mut c = 0;
    while c < 2 {
        let mut sq = 0;
        while sq < 64 {
            let mut to = 0;
            while to < 64 {
                let delta = (sq % 8) as i32 - (to % 8) as i32;
                if delta >= -1
                    && delta <= 1
                    && if c == 0 {
                        to / 8 > sq / 8
                    } else {
                        to / 8 < sq / 8
                    }
                {
                    result[c][sq] |= 1 << to;
                }
                to += 1;
            }
            sq += 1;
        }
        c += 1;
    }
    result
};

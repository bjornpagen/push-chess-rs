...
from .selfplay import collect, reanalyse
...
class TrainConfig:
    ...
    def __post_init__(self):
        ...
        for name in ("actors", "games", "simulations", "fast_simulations", "max_plies", "batch_size", "replay_capacity"):
...
def sample_distribution(samples):
    ...
    return {"action_count_quantiles": quantiles([len(s.ids) for s in samples]),
            ...
            "board_density_quantiles": quantiles([float(s.board[:12].sum()) for s in samples]),
            "policy_entropy_mean": float(np.mean([-np.sum(s.policy * np.log(s.policy.clip(1e-30))) for s in samples]))}


def train(directory, config=TrainConfig(), minutes=60, iterations=10000, resume=False, jit=True):
    ...
    with run_lock(directory), stop_signals() as stop:
        ...
        predictor = Predictor(learner.model, config.actors, jit=jit, workers=config.workers,
        ...
        try:
            ...
            for _ in range(iterations):
                ...
                if time.monotonic() >= collect_deadline:
                    break
                samples, games = collect(predictor, rng, games=config.games, actors=config.actors,
                    simulations=config.simulations, fast_simulations=config.fast_simulations,
                    full_fraction=config.full_fraction, max_plies=config.max_plies, curriculum=config.curriculum,
                    deadline=collect_deadline, stop=stop, progress=emit, archive=archive,
                    restart_fraction=config.restart_fraction, fast_explore=config.fast_explore, max_nodes=config.max_nodes)
                if not samples:
                    break
                for sample in samples:
                    sample.provenance = {"source": "selfplay", "iteration": iteration + 1, "policy_steps": learner.steps}
